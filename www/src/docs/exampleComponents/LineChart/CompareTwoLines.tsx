...
import { Dot, Line, LineChart, ReferenceLine, Tooltip, XAxis, YAxis } from 'recharts';
...
const App: FC = () => {
  ...
  const { yAxisMin, yAxisMax } = data.reduce(
    (acc, val) => {
      const cc = val.compareClose ?? 0;
      const mc = val.maxClose ?? 0;
      const nd = val.netDeposits ?? 0;

      return {
        yAxisMin: Math.min(acc.yAxisMin, cc, mc, nd),
        yAxisMax: Math.max(acc.yAxisMax, cc, mc, nd),
      };
    },
  ...
  return (
    <LineChart
      ...
      onMouseMove={({ activeIndex }) => setHoverIndex(Number(activeIndex ?? -1))}
    >
      <defs>
      ...
      <YAxis
        ...
        tickFormatter={(value: number) => formatUsdCompressed(value)}
        tick={{ fill: 'black' }}
        domain={[Math.max(0, 0.75 * yAxisMin), 1.25 * yAxisMax]}
      />
      {/** XAxis is required to show the lowest tick on the y-axis */}
      <XAxis dataKey="date" axisLine={false} tickLine={false} tick={false} tickSize={0} tickMargin={0} hide />
      <Tooltip
        offset={10}
      ...
      <Line
        type="linear"
        dataKey={point => point.netDeposits}
        name="Net deposits"
        dot={false}
        activeDot={false}
      ...
      <Line
        type="linear"
        dataKey={point => point.reinvestClose}
        name="Compare value"
        dot={false}
        activeDot={<Dot fill={compareColor} />}
