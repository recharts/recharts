...
testWithThemes('BarChart with barGap and a maxBarSize that does not clamp', async ({ mountStory }) => {
  const component = await mountStory('Bar.2774/BarChartWithBarGapAndNonClampingMaxBarSize');
  await expect(component).toHaveScreenshot();
});

testWithThemes('BarChart with barGap and a maxBarSize that clamps', async ({ mountStory }) => {
...
testWithThemes('BarChart with barGap and a clamping maxBarSize set on each Bar', async ({ mountStory }) => {
