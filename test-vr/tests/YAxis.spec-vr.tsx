...
testWithThemes('YAxis with default width', async ({ mountStory }) => {
  const component = await mountStory('YAxis/YAxisWithDefaultWidth');
  await expect(component).toHaveScreenshot();
});

testWithThemes('YAxis with width={80}', async ({ mountStory }) => {
...
testWithThemes('YAxis with width="auto"', async ({ mountStory }) => {
...
testWithThemes('YAxis with width="auto" and long tick values', async ({ mountStory }) => {
  const component = await mountStory('YAxis/YAxisWithWidthAutoAndLongTickValues');
  await expect(component).toHaveScreenshot();
});

testWithThemes('YAxis with width="auto" and tick=true', async ({ mountStory }) => {
  const component = await mountStory('YAxis/YAxisWithWidthAutoAndTickTrue');
  await expect(component).toHaveScreenshot(WIDTH_AUTO_TICK_DEFAULT_EQUALS_TRUE);
});

testWithThemes('YAxis with width="auto" and smaller tick fontsize', async ({ mountStory }) => {
  const component = await mountStory('YAxis/YAxisWithWidthAutoAndSmallerTickFontsize');
  await expect(component).toHaveScreenshot();
});

testWithThemes('YAxis with width="auto" and bigger tick fontsize', async ({ mountStory }) => {
  const component = await mountStory('YAxis/YAxisWithWidthAutoAndBiggerTickFontsize');
  await expect(component).toHaveScreenshot();
});

testWithThemes('YAxis with width="auto" and tick textAnchor = start', async ({ mountStory }) => {
  const component = await mountStory('YAxis/YAxisWithWidthAutoAndTickTextAnchorStart');
  await expect(component).toHaveScreenshot();
});

testWithThemes('YAxis with width="auto" and angled ticks', async ({ mountStory }) => {
  const component = await mountStory('YAxis/YAxisWithWidthAutoAndAngledTicks');
  await expect(component).toHaveScreenshot();
});

testWithThemes('YAxis with width="auto" and two-line ticks', async ({ mountStory }) => {
  const component = await mountStory('YAxis/YAxisWithWidthAutoTwoLineTicks');
  await expect(component).toHaveScreenshot();
});

testWithThemes('YAxis with default width and two-line ticks', async ({ mountStory }) => {
  const component = await mountStory('YAxis/YAxisWithTwoLineTicks');
  await expect(component).toHaveScreenshot();
});

testWithThemes('YAxis with width="auto" and long ticks', async ({ mountStory }) => {
  const component = await mountStory('YAxis/YAxisWithLongTicksAndWidthAuto');
  await expect(component).toHaveScreenshot();
});

testWithThemes('YAxis with width="auto" and long ticks and tick object', async ({ mountStory }) => {
...
testWithThemes(
  'YAxis with width="auto" and long ticks and tick object and smaller font size and maxLines=1',
  async ({ mountStory }) => {
