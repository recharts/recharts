...
describe('omnidoc - documentation consistency', () => {
  ...
  it.skip('should have autogenerated API docs for each exports that is not a type', () => {
    const projectSymbols = new Set(projectReader.getPublicComponentNames());
    for (const symbol of projectSymbols) {
      if (componentsExcludedFromOmnidoc.includes(symbol)) {
        continue;
      }
  ...
  it('should have a mapping defined in apiCates for each autogenerated API doc component because without this mapping, navigation will not render a link', () => {
    const componentsThatHaveApiCategoryMapping = apiCates.flatMap(cat => cat.items);

    for (const symbol of OMNIDOC_AUTOMATED_API_DOCS_COMPONENTS) {
