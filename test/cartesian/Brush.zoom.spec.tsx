...
describe('<Brush /> zoom mode', () => {
  ...
  it('lets explicit Brush limits override stricter chart-level limits', async () => {
    const { container } = render(
      <LineChart width={400} height={300} data={data}>
        <XAxis dataKey="name" />
        <YAxis />
        <Line dataKey="value" isAnimationActive={false} />
        <ZoomAndPan axis="x" maxZoom={2} scrollbars={false} />
        <Brush mode="zoom" maxZoom={10} wheelStep={2} x={0} y={250} width={200} height={40} />
        <CaptureZoom />
      </LineChart>,
    );
    ...
    fireEvent.wheel(brush, { deltaY: -100, clientX: 100, clientY: 260 });
    await waitFor(() => expect(zoomApi.viewport.x.end - zoomApi.viewport.x.start).toBeCloseTo(0.25, 5));
    expect(zoomApi.viewport.x.end - zoomApi.viewport.x.start).toBeLessThan(0.5);
  });
  ...
  it('does not let parent plot gestures also process a traveller drag', async () => {
    ...
    const { container } = render(
      <LineChart width={400} height={300} data={data}>
        ...
        <ZoomAndPan
          axis="x"
          initialZoom={{ x: { start: 0.2, end: 0.8 } }}
          onZoomChange={onZoomChange}
          scrollbars={false}
        />
        <Brush mode="zoom" x={100} y={100} width={200} height={40} />
        <CaptureZoom />
      </LineChart>,
    );
    ...
    if (traveller == null) {
      throw new Error('Expected a brush traveller');
    }
    fireEvent.pointerDown(traveller, { button: 0, pointerType: 'mouse', clientX: 140, clientY: 120 });
    fireEvent.mouseDown(traveller, { button: 0, clientX: 140, clientY: 120, pageX: 140, pageY: 120 });
    fireEvent.pointerMove(window, { pointerType: 'mouse', clientX: 160, clientY: 120 });
    fireEvent.mouseMove(traveller, { clientX: 160, clientY: 120, pageX: 160, pageY: 120 });
    fireEvent.pointerUp(window, { pointerType: 'mouse', clientX: 160, clientY: 120 });
    fireEvent.mouseUp(traveller);
  ...
  it('allows one horizontal and one vertical zoom brush in the same chart', async () => {
    ...
    const [horizontalBrush, verticalBrush] = container.querySelectorAll<SVGGElement>('.recharts-brush');
    const horizontalTraveller = horizontalBrush?.querySelector<SVGGElement>('.recharts-brush-traveller');
    const verticalTraveller = verticalBrush?.querySelector<SVGGElement>('.recharts-brush-traveller');
    if (horizontalTraveller == null || verticalTraveller == null) {
      throw new Error('Expected each brush to render a traveller');
    }
