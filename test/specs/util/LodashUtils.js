...
describe('ScatterChart of three dimension data', () => {
  ...
  it('LodashUtils', () => {
    expect(LodashUtils.isString('wrapper')).to.equal(true);
  });
  ...
  it('LodashUtils', () => {
    expect(LodashUtils.getPercentValue('25%', 1)).to.eql(0.25);
  });

  it('LodashUtils', () => {
    assert.isObject(LodashUtils.getElementOfObject(wrapper), 'getElementOfObject is an object');
  });

  it('LodashUtils', () => {    expect(LodashUtils.toPercentage(0.25)).to.eql('25%');
  });
