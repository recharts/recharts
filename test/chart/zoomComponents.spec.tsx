...
describe('zoom interaction components', () => {
  ...
  it('<DragToZoom/> leaves two-finger pinch to a coexisting <PinchZoom/>', () => {
  ...
  it('does not override or hijack embedded controls and interaction islands', async () => {
    const { wrapper, getByRole, getByTestId } = renderWith(
      <>
        <MouseWheelZoom />
        <PanOnDrag />
        <AxisZoom />
        <PinchZoom />
        <g data-recharts-zoom-interaction-island="true">
          <rect data-testid="interaction-island-target" />
        </g>
        <foreignObject>
          <button type="button" style={{ userSelect: 'text' }}>
            embedded control
          </button>
        </foreignObject>
      </>,
    );
    act(() => zoomApi.setViewport({ x: { start: 0.2, end: 0.6 }, y: { start: 0.25, end: 0.65 } }));
    await waitFor(() => expect(zoomApi.viewport.x.start).toBeCloseTo(0.2));

    const button = getByRole('button', { name: 'embedded control' });
    const islandTarget = getByTestId('interaction-island-target');
    const xAxis = wrapper.querySelector('.recharts-xAxis');
    if (xAxis == null) {
      throw new Error('Expected an x axis');
    }
    xAxis.setAttribute('role', 'slider');

    fireEvent.wheel(button, { deltaY: -100, clientX: 200, clientY: 150 });
    fireEvent.wheel(xAxis, { deltaY: -100, clientX: 200, clientY: 296 });
    fireEvent.pointerDown(islandTarget, { button: 0, pointerType: 'mouse', clientX: 180, clientY: 150 });
    fireEvent.pointerMove(window, { pointerType: 'mouse', clientX: 240, clientY: 150 });
    fireEvent.pointerUp(window, { pointerType: 'mouse', clientX: 240, clientY: 150 });
    fireEvent.pointerDown(xAxis, { button: 0, pointerType: 'mouse', clientX: 180, clientY: 296 });
    fireEvent.pointerMove(window, { pointerType: 'mouse', clientX: 240, clientY: 296 });
    fireEvent.pointerUp(window, { pointerType: 'mouse', clientX: 240, clientY: 296 });
    fireEvent.touchStart(islandTarget, {
      touches: [
        { clientX: 150, clientY: 150 },
        { clientX: 250, clientY: 150 },
      ],
    });
    fireEvent.touchMove(islandTarget, {
      touches: [
        { clientX: 100, clientY: 150 },
        { clientX: 300, clientY: 150 },
      ],
    });
    fireEvent.touchEnd(islandTarget, { touches: [] });
    fireEvent.touchStart(wrapper, { touches: [{ clientX: 150, clientY: 150 }] });
    fireEvent.touchEnd(wrapper, { touches: [] });
    fireEvent.touchStart(button, { touches: [{ clientX: 150, clientY: 150 }] });
    fireEvent.touchEnd(button, { touches: [] });
    fireEvent.touchStart(wrapper, { touches: [{ clientX: 150, clientY: 150 }] });
    fireEvent.touchEnd(wrapper, { touches: [] });
    flushRaf();

    expect(zoomApi.viewport).toMatchObject({
      x: { start: 0.2, end: 0.6 },
      y: { start: 0.25, end: 0.65 },
    });
    expect(button).toHaveStyle({ userSelect: 'text' });
  });

  it('<PinchZoom/> resets on a plain double-tap', async () => {
    const { wrapper } = renderWith(<PinchZoom />);
    act(() => zoomApi.setViewport({ x: { start: 0.2, end: 0.6 } }));
    await waitFor(() => expect(zoomApi.isZoomed).toBe(true));
