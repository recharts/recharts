...
describe('<Treemap />', () => {
  ...
  test('evaluates functional dataKey correctly', () => {
    const { container } = render(
      <Treemap
        width={500}
        height={250}
        ...
        content={(props: any) => {
          ...
          if (props.depth === 1) {
            return (
              <g className="custom-node" data-name={props.name} data-value={props.value}>
                <text>{props.name}</text>
              </g>
            );
          }
          return <g className="root-node" />;
        }}
      />,
    );
    ...
    // Validate that the value generated by dataKey function is exactly double baseValue
    const nodeA = container.querySelector('.custom-node[data-name="A"]');
    const nodeB = container.querySelector('.custom-node[data-name="B"]');
    expect(nodeA).toHaveAttribute('data-value', '20');
    expect(nodeB).toHaveAttribute('data-value', '40');
  });
