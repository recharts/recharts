...
import { roundPointsOrPaths } from '../__helpers__/roundPath';
...
describe('RadialBar animation', () => {
  ...
  describe('with isAnimationActive=false', () => {
    ...
    it('should render all bars without animation', () => {
      ...
      expect(roundPointsOrPaths(getRadialBarPathStrings(container))).toEqual(roundPointsOrPaths(expected));
    });
  });
  ...
  describe('tests that change data array', () => {
    ...
    it('should animate from 2 to 3 bars', async () => {
      const { container, animationManager } = renderTestCase();
      await prime(container, animationManager);
      expect(roundPointsOrPaths(await expectAnimatedRadialBarPaths(container, animationManager))).toEqual(
        roundPointsOrPaths([
          [
