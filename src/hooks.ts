...
export type UseZoomResult = {
  ...
  isZoomed: boolean;
  /** Replace the viewport. Omitted enabled axes reset to full; disabled axes retain their current window. */
...
export const useZoom = (): UseZoomResult => {
  ...
  const pan = useCallback(
    (deltaX: number, deltaY: number) =>
      dispatch(
        setZoom({
          x: zoomAxisEnabled(sharedAxis, 'x') ? panDimension(zoom.x, deltaX) : zoom.x,
          y: zoomAxisEnabled(sharedAxis, 'y') ? panDimension(zoom.y, deltaY) : zoom.y,
        }),
      ),
 ...
 * (`ZoomAndPan`, the granular interaction components, `Minimap`, `Brush mode="zoom"`), so custom
 * controls built on it stay in sync with all of them.
 *
 ...
 * without clobbering each other. Provided axes respect the chart's shared `axis`, `minZoom`, and
 * `maxZoom` settings. To reset an axis, pass a full-axis window from start 0 to end 1, or use
 * {@link useZoom}'s `reset()` (useZoom also exposes zoomIn/zoomOut/pan helpers, and its `setViewport`
...
export const useZoomState = (): [Required<ZoomViewport>, (viewport: ZoomViewport) => void] => {
  ...
  const setZoomWindows = useCallback(
    (viewport: ZoomViewport) => {
      ...
      const nextX =
        viewport.x != null && zoomAxisEnabled(sharedAxis, 'x')
          ? clampDimensionToLimits(windowToViewport(viewport.x), sharedLimits)
          : latestZoom.x;
      const nextY =
        viewport.y != null && zoomAxisEnabled(sharedAxis, 'y')
          ? clampDimensionToLimits(windowToViewport(viewport.y), sharedLimits)
          : latestZoom.y;

      dispatch(setZoom({ x: nextX, y: nextY }));
    },
    [dispatch, readState, sharedAxis, sharedLimits, zoom],
  );
