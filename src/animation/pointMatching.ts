...
/**
 * Extract a numeric value from a point for ordering comparisons (e.g. left-vs-right entry direction).
 * For custom function strategies, the index is unavailable in this context so 0 is passed as a placeholder.
 */
function getComparisonValue<T extends AnimatablePoint>(point: T, strategy: PointMatchingStrategy<T>): number | null {
  ...
  if (typeof strategy === 'function') {
    // Index is not available in comparison context; custom strategies should derive values from point data
...
/**
 * A function that computes the intermediate points for a single animation frame.
 *
 * @param currentPoints The target points for this data update
 * @param previousPoints The points from the previous frame
 * @param t Animation progress from 0 (previous positions) to 1 (current positions)
 * @returns The points to render at time t
 */
export type AnimationStepFunction<T extends AnimatablePoint = AnimatablePoint> = (
  currentPoints: ReadonlyArray<T>,
  previousPoints: ReadonlyArray<T>,
  t: number,
) => ReadonlyArray<T>;
...
/**
 * The legacy Recharts animation step function.
 * Matches points by array index with proportional stretching when array lengths differ.
 */
export function indexMatchLinearInterpolation<T extends AnimatablePoint>(
  currentPoints: ReadonlyArray<T>,
  previousPoints: ReadonlyArray<T>,
  t: number,
): T[] {
  ...
  const prevPointsDiffFactor = previousPoints.length / currentPoints.length;
  return currentPoints.map((entry, index): T => {
    const prevPointIndex = Math.floor(index * prevPointsDiffFactor);
    const prev = previousPoints[prevPointIndex];
    if (prev) {
...
/**
 * Time-series animation step function.
 * Matches points by x-coordinate so existing points slide smoothly,
 * new points animate in from the edges, and removed points slide off-screen.
 */
export function xMatchLinearInterpolation<T extends AnimatablePoint>(
  currentPoints: ReadonlyArray<T>,
  previousPoints: ReadonlyArray<T>,
  t: number,
): T[] {
  const { matched, removed } = matchPointsWithRemovals(currentPoints, previousPoints, 'x');
  return interpolateMatchedPoints(matched, removed, t, interpolate, true);
}
