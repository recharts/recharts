...
export interface BarProps<DataPointType = unknown, ValueAxisType = unknown> extends ZIndexable {
  className?: string;
  index?: Key;
  /**
   ...
   */
  zIndex?: number;
  // Bar does not have data prop for historical reasons, but we could put it back now: https://github.com/recharts/recharts/issues/5566
...
type InternalBarProps<DataPointType = unknown, ValueAxisType = unknown> = {
  /*
...
export type Props<DataPointType = unknown, ValueAxisType = unknown> = Partial<BarEvents> &
  BarProps<DataPointType, ValueAxisType> &
  Omit<BarSvgProps, keyof BarEvents>;

type InternalProps<DataPointType = unknown, ValueAxisType = unknown> = BarSvgProps &
  InternalBarProps<DataPointType, ValueAxisType>;
...
type BarBackgroundProps<DataPointType = unknown, ValueAxisType = unknown> = {
  background?: ActiveShape<BarShapeProps, SVGPathElement>;
  data: ReadonlyArray<BarRectangleItem> | undefined;
  dataKey: DataKey<any> | undefined;
  allOtherBarProps: InternalProps<DataPointType, ValueAxisType>;
};

function BarBackground<DataPointType = unknown, ValueAxisType = unknown>(
  props: BarBackgroundProps<DataPointType, ValueAxisType>,
) {
  ...
  return (
    <ZIndexLayer zIndex={getZIndexFromUnknown(backgroundFromProps, DefaultZIndexes.barBackground)}>
      {data.map((entry: BarRectangleItem, i: number) => {
        ...
        const barRectangleProps: BarRectangleProps<DataPointType, ValueAxisType> = {
        ...
        return <BarRectangle<DataPointType, ValueAxisType> key={`background-bar-${i}`} {...barRectangleProps} />;
      })}
    </ZIndexLayer>
  );
}

type BarRectanglesProps<DataPointType = unknown, ValueAxisType = unknown> = InternalProps<
  DataPointType,
  ValueAxisType
> & {
  data: ReadonlyArray<BarRectangleItem> | undefined;
...
function BarRectangleWithActiveState<DataPointType = unknown, ValueAxisType = unknown>(props: {
    ...
    return (
      <ZIndexLayer zIndex={DefaultZIndexes.activeBar}>
        <BarStackClipLayer index={index}>
          <BarRectangle<DataPointType, ValueAxisType>
            {...baseProps}
...
function BarRectangleNeverActive<DataPointType = unknown, ValueAxisType = unknown>(props: {
  shape: ActiveShape<BarShapeProps, SVGPathElement> | undefined;
  baseProps: WithoutId<SVGPropsNoEvents<BarRectanglesProps>>;
  entry: BarRectangleItem;
  index: number;
...
function BarRectangles<DataPointType = unknown, ValueAxisType = unknown>({
  data,
  props,
}: {
  data: ReadonlyArray<BarRectangleItem> | undefined;
  props: BarRectanglesProps<DataPointType, ValueAxisType>;
}) {
  const { id, ...baseProps } = (svgPropertiesNoEvents(props) as any) ?? {};
...
function RectanglesWithAnimation<DataPointType = unknown, ValueAxisType = unknown>({
  props,
...
function RenderRectangles<DataPointType = unknown, ValueAxisType = unknown>(
  props: InternalProps<DataPointType, ValueAxisType>,
) {
...
class BarWithState<DataPointType = unknown, ValueAxisType = unknown> extends PureComponent<
...
type BarImplProps<DataPointType = unknown, ValueAxisType = unknown> = Omit<
  InternalBarProps<DataPointType, ValueAxisType>,
  'layout' | 'data'
> & { children?: ReactNode };

function BarImpl<DataPointType = unknown, ValueAxisType = unknown>(props: BarImplProps<DataPointType, ValueAxisType>) {
  const {
    xAxisId,
    yAxisId,
    hide,
...
function BarFn<DataPointType = unknown, ValueAxisType = unknown>(outsideProps: Props<DataPointType, ValueAxisType>) {
  ...
  return (
    <RegisterGraphicalItemId id={props.id} type="bar">
      {id => (
        <>
          <SetLegendPayload legendPayload={computeLegendPayloadFromBarData(props as any)} />
 ...
 */
export const Bar = React.memo(BarFn, propsAreEqual) as (<DataPointType = unknown, ValueAxisType = unknown>(
