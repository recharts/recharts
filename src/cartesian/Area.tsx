 ...
 */
type BaseValueCoordinate<DataPointType> = NullableCoordinate & { payload: DataPointType | undefined };

type BaseLineType = number | ReadonlyArray<NullableCoordinate> | undefined;
 ...
 */
interface AreaProps<DataPointType = any, ValueAxisType = any> extends DataProvider<DataPointType>, ZIndexable {
  /**
...
function AreaWithAnimation<DataPointItem>({
  ...
  previousBaselineRef: MutableRefObject<BaseLineType | null>;
}) {
  const {
    points,
    baseLine,
...
function RenderArea<DataPointType>({
  ...
  const previousBaselineRef = useRef<BaseLineType | null>();
