...
import { selectTooltipPointerCoordinate } from '../state/selectors/tooltipSelectors';
...
import { panDimension } from '../util/zoom/zoomActions';
...
export type ScrollbarStyle = {
  ...
  /** Accessible names supplied by the application for the focusable axis scrollbars. */
  ariaLabels?: Partial<Record<ZoomDimension, string>>;
};
...
function AxisScrollbar({
  ...
  const onKeyDown = (event: React.KeyboardEvent<HTMLDivElement>) => {
    let next: AxisViewport | undefined;
    const towardEnd = horizontal ? event.key === 'ArrowRight' : event.key === 'ArrowDown';
    const towardStart = horizontal ? event.key === 'ArrowLeft' : event.key === 'ArrowUp';
    if (towardStart || towardEnd) {
      const screenDirection = towardEnd ? 1 : -1;
      next = panDimension(viewport, screenDirection * (flipped ? -1 : 1) * 0.1);
    } else if (event.key === 'Home') {
      const startRatio = flipped ? maxStartRatio : 0;
      next = { startRatio, endRatio: startRatio + windowRatio };
    } else if (event.key === 'End') {
      const startRatio = flipped ? 0 : maxStartRatio;
      next = { startRatio, endRatio: startRatio + windowRatio };
    }
    if (next == null) {
      return;
  ...
  return (
    <div
      ...
      aria-valuemin={0}
      aria-valuemax={maxStartRatio}
      aria-valuenow={displayStartRatio}
...
export function ZoomScrollbars({
  ...
  const activeTooltipCoordinate = useAppSelector(selectTooltipPointerCoordinate);
    ...
    <>
      {showX && (
        <AxisScrollbar
          ...
          flipped={xFlipped}
          ariaLabel={scrollbarStyle.ariaLabels?.x ?? options.scrollbarAriaLabels?.x}
        />
      )}
      {showY && (
        <AxisScrollbar
          ...
          refreshActivePointer={refreshActivePointer}
          flipped={yFlipped}
          ariaLabel={scrollbarStyle.ariaLabels?.y ?? options.scrollbarAriaLabels?.y}
        />
      )}
    </>
  );
