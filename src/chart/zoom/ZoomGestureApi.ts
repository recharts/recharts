...
const INTERACTIVE_ZOOM_TARGET_SELECTORS = [
  ...
  '[role="slider"]',
  '[role="scrollbar"]',
  '[data-recharts-zoom-scrollbar]',
  '[data-recharts-zoom-interaction-island]',
] as const;

const INTERACTIVE_ZOOM_TARGET_SELECTOR = INTERACTIVE_ZOOM_TARGET_SELECTORS.join(', ');
const INTERACTIVE_ZOOM_TARGET_AND_DESCENDANT_SELECTOR = INTERACTIVE_ZOOM_TARGET_SELECTORS.flatMap(selector => [
  selector,
  `${selector} *`,
]).join(', ');
...
export function isInteractiveZoomTarget(target: EventTarget | null): boolean {
  ...
  return target.closest(INTERACTIVE_ZOOM_TARGET_SELECTOR) != null;
}
...
function createTouchDecorationStyles(): HTMLStyleElement {
  ...
  stylesheet.textContent = `
    .${TOUCH_DECORATION_CLASS} .recharts-surface:not(:has(${INTERACTIVE_ZOOM_TARGET_SELECTOR})),
    .${TOUCH_DECORATION_CLASS} .recharts-surface :not(:where(${INTERACTIVE_ZOOM_TARGET_AND_DESCENDANT_SELECTOR})):not(:has(${INTERACTIVE_ZOOM_TARGET_SELECTOR})) {
...
export function suppressTouchBrowserDecorations(element: HTMLElement): () => void {
  ...
  const targets = Array.from(
    element.querySelectorAll<HTMLElement | SVGElement>('.recharts-surface, .recharts-surface *'),
  ).filter(
    target => !isInteractiveZoomTarget(target) && target.querySelector(INTERACTIVE_ZOOM_TARGET_SELECTOR) == null,
  );
