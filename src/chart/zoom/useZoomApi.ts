...
function getAxisRegionFromTarget(target: EventTarget | null | undefined): PointerRegion | null {
  if (!(target instanceof Element)) {
    return null;
  }
  const stableTouchTarget = target
    .closest('[data-recharts-zoom-axis-target]')
    ?.getAttribute('data-recharts-zoom-axis-target');
  if (stableTouchTarget === 'x' || target.closest('.recharts-xAxis') != null) {
    return 'xAxis';
  }
  if (stableTouchTarget === 'y' || target.closest('.recharts-yAxis') != null) {
    return 'yAxis';
  }
  return null;
}
  ...
  return useMemo<ZoomGestureApi | null>(() => {
    ...
    return {
      ...
      pointerRegion: (clientX, clientY, target) => {
        ...
        const axisRegion = getAxisRegionFromTarget(target);
        if ((axisRegion === 'xAxis' && live.current.hasXAxis) || (axisRegion === 'yAxis' && live.current.hasYAxis)) {
          return axisRegion;
        }
        const o = live.current.offset;
        if (o == null) {
      ...
      refreshActivePointer: () => {
        ...
        // which selectTooltipPointerCoordinate keeps aligned with its data point as the chart pans.
