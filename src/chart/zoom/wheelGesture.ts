...
export const installWheelGesture: ZoomGestureInstaller = api => {
  const onWheel = (event: WheelEvent) => {
    ...
    // The dedicated ZoomScrollbar wheel installer deliberately owns its own scrollbar region. All
    // other embedded controls and interaction islands keep their native wheel behavior.
    const ownsScrollbarWheel =
      (region === 'xScrollbar' || region === 'yScrollbar') && options.wheelRegions?.includes(region) === true;
    if (isInteractiveZoomTarget(event.target) && !ownsScrollbarWheel) {
      return;
    }
