...
type ZoomGestureOptions = {
  ...
  scrollbars?: boolean;
  /**
   * Base fraction of the visible window used for keyboard panning. Shift + arrow keys pan by
   * `panStep * panFastMultiplier`. @defaultValue 0.1
   */
  panStep?: number;
  /** Multiplier applied to the base `panStep` for Shift + arrow-key panning. @defaultValue 2.5 */
...
export type ZoomOptions = ZoomGestureOptions & {
  ...
  /**
   * Called whenever an uncontrolled viewport changes. In controlled mode, called once with each
   * proposed viewport; applying the `viewport` prop or restoring a rejected proposal stays silent.
   * Both dimensions are provided in `[0, 1]` window form.
   */
  onZoomChange?: (viewport: Required<ZoomViewport>) => void;
  /** Localized accessible names for the optional x/y scrollbars. */
  scrollbarAriaLabels?: Partial<Record<ZoomDimension, string>>;
};
...
export type ResolvedZoomOptions = Required<ZoomGestureOptions> &
  Pick<ZoomOptions, 'viewport' | 'initialZoom' | 'onZoomChange' | 'scrollbarAriaLabels'> & {
    /**
