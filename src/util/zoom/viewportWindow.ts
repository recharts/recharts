...
export type ViewportWindowHandle =
  'body' | 'left' | 'right' | 'top' | 'bottom' | 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right';
...
export function hitTestViewportWindow(
  ...
  const onLeft = distanceLeft <= handleSize && distanceLeft <= distanceRight;
  const onRight = distanceRight <= handleSize && distanceRight < distanceLeft;
  const onTop = distanceTop <= handleSize && distanceTop <= distanceBottom;
  const onBottom = distanceBottom <= handleSize && distanceBottom < distanceTop;
