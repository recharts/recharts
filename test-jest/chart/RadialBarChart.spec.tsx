...
describe('<RadialBarChart />', () => {
  ...
  it('Should fill the RadialBar with color when fill property is present in data', () => {
    const { container } = render(
      <RadialBarChart
        width={500}
        height={300}
    ...
    expect(container.querySelector('.recharts-radial-bar-sector')).toHaveAttribute('fill', data[0].fill);
  });

  it('Should fill the RadialBar with the default color when fill property is not present in data', () => {
    const { container } = render(
      <RadialBarChart
        width={500}
        height={300}
        ...
        outerRadius={140}
        barSize={10}
        data={[{ name: '18-24', uv: 31.47, pv: 2400 }]}
      >
        <RadialBar background dataKey="uv" isAnimationActive={false} />
      </RadialBarChart>,
    );

    expect(container.querySelector('.recharts-radial-bar-sector')).not.toHaveAttribute('fill');
  });

  it('Renders tooltip when add a Tooltip element', async () => {
    const { container } = render(
      <RadialBarChart
        width={500}
        height={300}
    ...
    expect(container.querySelector('.recharts-default-tooltip')).toBeInTheDocument();
