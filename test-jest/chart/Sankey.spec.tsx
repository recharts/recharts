...
describe('<Sankey />', () => {
  ...
  it('re-renders links and nodes when data changes', () => {
    ...
    rerender(<Sankey data={newData} />);

    setTimeout(() => {
      expect(container.querySelectorAll('.recharts-sankey-node').length).to.equal(49);
      expect(container.querySelectorAll('.recharts-sankey-link').length).to.equal(69);
    }, 1000);
  });
});
