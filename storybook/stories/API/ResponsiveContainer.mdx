...
import * as ResponsiveContainerStories from './ResponsiveContainer.stories';
...
If you're using ResponsiveContainer and you're rendering on the server (SSR) as most JavaScript
frameworks do, then initially on the Browser, the chart will not be shown until after hydration.
...
> [content layout shift](https://web.dev/cls) that can be annoying for your users.
...
The ResponsiveContainer can be used to render all types of charts

## Properties

Properties in the groups Other and Internal are not recommended to be used.

<ArgTypes of={ResponsiveContainerStories} sort={'requiredFirst'} /> 