...
The PieChart can be used with the following child components: `<PolarAngleAxis />`, `<PolarRadiusAxis />`, `<PolarGrid />`, `<Legend />`, `<Tooltip />`, `<Pie />`, `<Customized />` or validate svg elements.
