...
The RadarChart can be used with the following child components: `<PolarAngleAxis />`, `<PolarRadiusAxis />`, `<PolarGrid />`, `<Legend />`, `<Tooltip />`, `<Radar />`, `<Customized />` or `<validate svg elements... />`.
